...
export default function UrenBetaaldStats() {
  ChartJS.register(
    CategoryScale,
    LinearScale,
    BarElement,
    ...
    datasets: [
      {
        label: "Betaalde uren",
        data: [
          urenStats?.totaal2020._sum.urenBetaald.toFixed(2),
          urenStats?.totaal2021._sum.urenBetaald.toFixed(2),
  ...
  return (
    <div className="w-[100%]  group-focus-visible:flex bg-slate-100 mt-4 rounded-md p-4  mr-2 row-span-1">
      <h1 className="text-xl font-semibold text-sky-500">Uren betaald:</h1>

      <Bar options={options} data={data} />
      <div className="grid grid-cols-2 mt-4">
        <p className="p-2 bg-slate-200 rounded-md mb-2 mx-2">
          2020: {urenStats?.totaal2020._sum.urenBetaald.toFixed(2)}
        </p>
        <p className="p-2 bg-slate-200 rounded-md mb-2 mx-2">
          2021: {urenStats?.totaal2021._sum.urenBetaald.toFixed(2)}
        </p>
        <p className="p-2 bg-slate-200 rounded-md mb-2 mx-2">
          2022: {urenStats?.totaal2022._sum.urenBetaald.toFixed(2)}
        </p>
        <p className="p-2 bg-slate-200 rounded-md mb-2 mx-2">
          Totaal: {urenStats?.totaal._sum.urenBetaald.toFixed(2)}
        </p>
      </div>
